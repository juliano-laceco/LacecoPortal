...
// Create Zustand store for efficient state management
const useSheetStore = create((set, get) => ({
  cellContents: {},
  history: [{}],
  historyIndex: 0,

  setCellContents: (newCellContents) => set({ cellContents: newCellContents }),
  setHistory: (newHistory) => set({ history: newHistory }),
  setHistoryIndex: (newIndex) => set({ historyIndex: newIndex }),

  saveHistory: (newCellContents) => {
    const { history, historyIndex } = get();
    const newHistory = history.slice(0, historyIndex + 1);
    set({
      history: [...newHistory, newCellContents],
      historyIndex: newHistory.length, // Set index to the end of the new history
    });
  },
}));
...
const useSheet = (
  ...
  // Initialize cellContents and history if empty
  useEffect(() => {
    ...
    if (history.length === 1 && Object.keys(history[0]).length === 0) {
      setHistory([initialCellContents]);
    }
  }, [initialCellContents, setCellContents, cellContents, setHistory, history]);
  ...
  }, [historyIndex, history, setHistoryIndex, setCellContents]);
    ...
    (e) => {
      if ((e.key === "c" || e.key === "x") && e.ctrlKey) {
        ...
        const content = selectedCells.map((cell) => {
          ...
          const cellContent =
            cellContents[cellKey] !== undefined ? cellContents[cellKey] : "";
        ...
        switch (e.key) {
          ...
          case "ArrowDown":
            ...
            break;
          case "ArrowLeft":
            newEndCell.col = Math.max(numberOfUneditableCells, endCell.col - 1);
            break;
          case "ArrowRight":
        ...
        if (e.key.length === 1) {
          ...
          const existingContent = cellContents[cellKey] || "";
          const newContent = existingContent + e.key; // Append the new character
          ...
          if (!editableCell) {
            switch (e.key) {
              case "ArrowUp":
                ...
                break;
              case "ArrowDown":
                newSelectedCell.row = Math.min(
                  numRows - 1,
                  selectedCells[0].row + 1
                );
                break;
              case "ArrowLeft":
                newSelectedCell.col = Math.max(
                  numberOfUneditableCells,
                  selectedCells[0].col - 1
                );
                break;
              case "ArrowRight":
                newSelectedCell.col = Math.min(
                  numCols - 1,
                  selectedCells[0].col + 1
                );
                break;
  ...
  const handleCellBlur = useCallback(
    ...
    [cellContents, saveHistory, setCellContents]
  );
